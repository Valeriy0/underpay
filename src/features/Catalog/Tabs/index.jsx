    ...
    return (
        <div className="flex-shrink-0 w-full py-[1.6rem] px-[0.8rem] flex items-center justify-start space-x-[0.4rem] overflow-x-auto overflow-y-hidden invisible-scrollbar">
            ...
            {!isOpenedSearch && !!list && list?.map((item, itemList) => {
                ...
                return (
                    <button onClick={() => setChoosed(item?.name)} className={`whitespace-nowrap h-[4.8rem] flex items-center justify-center px-[2rem] space-x-[0.8rem] rounded-[3.2rem] ${isChoosed ? 'bg-white text-[#101010]' : 'bg-onBg text-white'}`} key={itemList}>
                        <img className="h-[1.6rem]" src={item?.icon} alt={`tab_${item?.name}`} />
